...
class SharedAccountHistory_model extends CI_Model {
  ...
  private function _get_shared_account_history_datatables_query_sent()
    ...
    // Action
    private function _get_shared_account_history_datatables_query_action_limit()
    {
    ...
    function get_shared_account_history_datatables_action_limit()
    {
        $this->_get_shared_account_history_datatables_query_action_limit();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    public function count_filtered_action_limit()
    {
        $this->_get_shared_account_history_datatables_query_action_limit();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_action_limit()
    {
        $this->db->from($this->table);
        $this->db->where('STATUS_CONFIRMATION', 'RECEIVED');
        return $this->db->count_all_results();
