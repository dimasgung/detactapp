...
class AnalyticResult extends CI_Controller {
    ...
    function submitSharedAccountData(){
        ...
        $logintracking_history_data = $this->LoginTrackingHistory_model->get_logintracking_history_id_by_attemptdate_and_application($attemptdate, $application);

        if(isset($logintracking_history_data)){
            foreach ($logintracking_history_data as $logintracking_history_item) {        
                $this->LoginTrackingHistory_model->update_status_to_done_logintracking_history_by_id($logintracking_history_item['LOGINTRACKING_HISTORY_ID']);
            }
        }
